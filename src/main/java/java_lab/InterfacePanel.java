...
public class InterfacePanel extends JPanel {
    ...
    private static boolean printHistoryOnStart = true; //config
    ...
    InterfacePanel() {
        ...
        //print history on start
        if(printHistoryOnStart)
            txt_history.setText(new Logic().history());

    }


    private static void addHistory(String source, String result) {
        history_list.add(0, source + " --> " + result);
        if(history_list.size() > 5)
            history_list.remove(5);
    }
